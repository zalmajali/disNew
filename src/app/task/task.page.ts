...
export class TaskPage implements OnInit {
  ...
  public completeSuccessfully:any;
  public completeNotSuccessfully:any;
  public engineerStatusId:any=1;
  public callDate:any;
  public showCADTime:any="unShowAppointment";
  public errorNoSelectParts:any;
  //dark mode classs
  public darkMode:any;
...
//----------------------------------------------------
  ...
  initialiseTranslation(){
    ...
    this.translate.get('complete_successfully').subscribe((res: string) => {
      this.completeSuccessfully = res;
    });
    this.translate.get('complete_not_successfully').subscribe((res: string) => {
      this.completeNotSuccessfully = res;
    });
    this.translate.get('error_no_select_parts').subscribe((res: string) => {
      this.errorNoSelectParts = res;
    });
  }
  //open complete model
  ...
  //get The Options
  getTheOptions(evant:any){
    this.engineerStatusId = evant.detail.value;
  }
  ...
  selectTicketsStatus(evant:any){
    ...
    if(this.ticketsStatusSelectVal == 10){
      ...
      this.showCADTime = "unShowAppointment";
    }else{
      this.showCompleteButton = 0;
      this.showCADTime = "unShowAppointment";
      this.engineerStatusId = 0;
    }
    if(this.ticketsStatusSelectVal == 5){
      this.engineerStatusId = 0;
      this.showCADTime = "showAppointment";
    }else{
      this.showCADTime = "unShowAppointment";
    }
  }
  ...
  //function to get call date
  checkTicketCallDate(evant:any){
    this.callDate = evant;
  }
  async ngOnInit() {
  ...
  async updateStatusTicket(){
    this.checkInternetData();
      let checkAllValues = 1;
    if(this.ticketsStatusSelectVal == 10 && this.ticketPartsArray.length != this.partsSelectPartStatusValArray.length)
      checkAllValues = 0;
    if(checkAllValues == 1){
      const loading = await this.loading.create({
        cssClass: 'my-custom-class',
      ...
      return false;
      let sendValues = {'ticket_id':this.mapId,'parts':this.partsSelectPartStatusValArray, 'note':this.ticketNote,'eta':this.ticketEta,'status_id':this.ticketsStatusSelectVal,'image':this.signatureImg,'na':this.valuseNorA,'plos':this.valuePlos,"lat":this.lat,"lng":this.lng,'engineer_status_id':this.engineerStatusId,'call_date':this.callDate};
      ...
      this.message = this.errorNoSelectParts;
      this.displayResult(this.message);
    }
  }
